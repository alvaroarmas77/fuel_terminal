...
class AccessControlTool(BaseTool):
    ...
    description: str = (
        "Consulta la base de datos de seguridad para validar si un conductor tiene "
        "permisos de entrada activos, seguro SCTR vigente y sin bloqueos administrativos."
    )
    ...
    def _run(self, driver_id: str, terminal_id: str = "TERM-01") -> str:
        ...
        try:
            ...
            if datos["status"] == "Bloqueado":
                return f"ALERTA: Acceso Denegado. El conductor {datos['nombre']} tiene un bloqueo administrativo."
